...
// 临时创建processAsyncRoutes和transformToVueRoutes函数
const processAsyncRoutes = (routes: any[]) => routes
const transformToVueRoutes = (routes: any[]) => routes

// 临时创建store函数
const usePermissionStoreWithOut = () => ({
  getIsRoutesLoaded: { value: false },
  setStaticRoutes: () => {},
  setDynamicRoutes: () => {},
  setIsRoutesLoaded: () => {},
  reset: () => {},
...
export const initDynamicRoutes = async (
  ...
  try {
    ...
    permissionStore.setStaticRoutes()
    ...
    permissionStore.setIsRoutesLoaded()
  } catch (error) {
    // 只在最终失败时设置标志
